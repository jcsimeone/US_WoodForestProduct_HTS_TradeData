...
wood_hts_to_query <- read.csv(paste0(dataPath, "WoodForestProducts_HTS_to_Query.csv"), stringsAsFactors = FALSE)
...
url <-"https://api.census.gov/data/timeseries/intltrade/imports/hs?key=",API_key,"&get=MONTH%2CI_COMMODITY_LDESC%2CDISTRICT%2CCTY_CODE%2CCTY_NAME%2CDIST_NAME%2CGEN_VAL_MO%2CCON_VAL_MO%2CGEN_QY1_MO%2CCON_QY1_MO%2CUNIT_QY1&YEAR=2022&COMM_LVL=HS10&I_COMMODITY=940330%2A"
batch <- httr::GET(url)
...
for(hts in wood_hts_to_query$HS){
 print(hts)
